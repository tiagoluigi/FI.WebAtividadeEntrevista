...
$(document).ready(function () {    
    json = JSON.stringify(obj.Beneficiarios);
    ...
    $('#formCadastro').submit(function (e) {
            ...
            const dados = {
                ...
                CPF: $("#CPF").val(),
                Beneficiarios: ObterRegistros().length == 0 ? obj.Beneficiarios : ObterRegistros() // já é um array de objetos
             };
...
function ModalBeneficiarios() {
    ...
    $('body').append($modal);
    MontarGrid()
    $('#IncluirBeneficiario').on('click', AdicionarBeneficiario)
                ...
                display: function (data) {
                    ...
                    const $btnExcluir = $('<button>')
                        ...
                        .click(function () {
                            ...
                            json = json.filter(b => b.IDBeneficiario !== data.record.IDBeneficiario)
                        });
...
//                    }
//                });
//                json.push({ CPFBeneficiario: cpf, NomeBeneficiario: nome });
//            }
//            else {
...
function AdicionarBeneficiario() {
    $('#IncluirBeneficiario').off('click').click(function () {
        ...
        if (idEmEdicao !== null) {
            ...
            // Atualiza o array json
            const index = json.findIndex(b => b.IDBeneficiario == idEmEdicao);
            if (index > -1) {
                json[index].CPFBeneficiario = cpf;
                json[index].NomeBeneficiario = nome;
            }
            ...
            // Modo inclusão: verificar duplicidade
            if (cpfExisteNoGrid(cpf)) {
                ModalDialog('Erro', 'CPF já foi inserido');
                $('#CPFBeneficiario').val('');
                $('#NomeBeneficiario').val('');
    ...
    $('#Grid .jtable-data-row').each(function () {
        ...
        if (dados && dados.CPFBeneficiario && dados.NomeBeneficiario) {
            registros.push({
                ...
                IDBeneficiario: dados.IDBeneficiario
            });
        }
    });
...
function cpfExisteNoGrid(cpf) {
    ...
    $('#Grid').find('.jtable-data-row').each(function () {        
        const linhaCPF = $(this).find('td').first().text().trim();
        const linhaID = $(this).attr('data-record-key');

        // Ignora o CPF que está sendo editado
        if (linhaCPF === cpf && linhaID !== String(idEmEdicao)) {
            existe = true;
