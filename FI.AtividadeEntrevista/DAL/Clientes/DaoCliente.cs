...
namespace FI.AtividadeEntrevista.DAL
{
    ...
    internal class DaoCliente : AcessoDados
    {
        ...
        internal long Incluir(DML.Cliente cliente)
        {
            ...
            var id = int.Parse(ds.Tables[0].Rows[0][0].ToString());
            DaoBeneficiarios daoBeneficiarios = new DaoBeneficiarios();

            if (cliente.Beneficiarios != null) {
                foreach (var beneficiario in cliente.Beneficiarios)
                {
                    beneficiario.IDCliente = id;
                    daoBeneficiarios.Incluir(beneficiario);
                }
            }          
            long ret = 0;
        ...
        internal void Alterar(DML.Cliente cliente)
        {
            ...
            var lista = beneficiarios.Consultar(cliente.Id);
            if (cliente.Beneficiarios != null)
            {
                var idsCliente = cliente.Beneficiarios.Select(x => x.IDBeneficiario).ToList();

                var remover = lista
                    .Where(b => !idsCliente.Contains(b.IDBeneficiario))
                    .ToList();
                foreach (var r in remover)
                {
                    beneficiarios.ExcluirBeneficiario(r.IDBeneficiario);
                }
                foreach (var item in cliente.Beneficiarios)
                {
                    if (beneficiarios.ExisteBeneficiario(item.IDBeneficiario))
                    {
                        beneficiarios.Alterar(item);
                    }
                    else
                    {
                        item.IDCliente = int.Parse(cliente.Id.ToString());
                        beneficiarios.Incluir(item);
                    }
